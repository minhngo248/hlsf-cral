...
class GAUSSIAN_MODEL_2(LSF_MODEL):
    ...
    Gaussian model
    """
    def __init__(self, lsf_data, listLines=None, _coeff: dict=None) -> None:
        ...
        if (_coeff == None):
            ...
            popt = [[popt[0], popt[1]], [popt[2], popt[3]], [popt[4], popt[5]]]
            li = ["Amplitude", "Mean", "Sigma"]
            self._coeff = dict(zip(li, popt))
    ...
    def evaluate_intensity(self, w_0, waves):
        ...
        """
        coeff = self._coeff.values()
        coeff = [i for item in coeff for i in item]
        eval_intensity = poly_gauss(waves-w_0, w_0, *coeff)

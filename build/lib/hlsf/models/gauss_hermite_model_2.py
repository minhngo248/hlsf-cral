...
class GAUSS_HERMITE_MODEL_2(LSF_MODEL):
    ...
    def __init__(self, lsf_data, deg, listLines=None, _coeff=None) -> None:
        ...
        if (_coeff == None):
            li = [f'Par{i}' for i in range(deg+1)]
            ...
            lis = [[popt[2*i], popt[2*i+1]] for i in range(int(len(popt)/2))]
            self._coeff = dict(zip(li, lis))
    ...
    def evaluate_intensity(self, w_0, waves):
        ...
        eval_intensity = poly_gauss_hermite(waves-w_0, w_0, self.deg, *coeff)
